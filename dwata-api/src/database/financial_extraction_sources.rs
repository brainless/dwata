...
pub async fn mark_source_processed(
    ...
    let id: i64 = conn.query_row(
        "INSERT INTO financial_extraction_sources
         (data_source_type, data_source_id, extraction_job_id, extracted_at, transaction_count)
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT (data_source_type, data_source_id)
         DO UPDATE SET
            extraction_job_id = excluded.extraction_job_id,
            extracted_at = excluded.extracted_at,
            transaction_count = excluded.transaction_count
         RETURNING id",
        params![data_source_type, data_source_id, extraction_job_id, now, transaction_count],
        |row| row.get(0),
    )?;
...
pub async fn is_source_processed(
    ...
    let count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM financial_extraction_sources
         WHERE data_source_type = ? AND data_source_id = ?",
...
pub async fn get_processed_sources(
    ...
    let mut stmt = conn.prepare(
        "SELECT data_source_id FROM financial_extraction_sources
         WHERE data_source_type = ?",
    )?;

    let rows = stmt.query_map([data_source_type], |row| row.get::<_, String>(0))?;
...
pub async fn list_processed_sources(
    ...
    let mut stmt = conn.prepare(
        "SELECT id, data_source_type, data_source_id, extraction_job_id, extracted_at, transaction_count
