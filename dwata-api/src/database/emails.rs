    ...
    task::spawn_blocking(move || {
        ...
        let rows = stmt.query_map(rusqlite::params![after_id, limit as i64], |row| {
            let email = Email {
                ...
                from_address: row.get(6)?,
                from_name: row.get(7)?,
                to_addresses: serde_json::from_str(&row.get::<_, String>(8)?).unwrap_or_default(),
                cc_addresses: serde_json::from_str(&row.get::<_, String>(9)?).unwrap_or_default(),
                bcc_addresses: serde_json::from_str(&row.get::<_, String>(10)?).unwrap_or_default(),
                reply_to: row.get(11)?,
                date_sent: row.get(12)?,
                date_received: row.get(13)?,
                body_text: row.get(14)?,
