...
use shared_types::{
    extraction::{Attachment, ExtractedEntity, ExtractionInput},
    ArchiveType, DataType, EmailAttachment, ExtractionJobStatus, ExtractionSourceType,
    ExtractorType,
};
...
impl ExtractionManager {
    ...
    async fn run_extraction_job(db_conn: &AsyncDbConnection, job_id: i64) -> Result<()> {
        let job = jobs_db::get_extraction_job(db_conn.clone(), job_id).await?;

        match &job.source_type {
            ExtractionSourceType::EmailAttachment => {
                let source_config: shared_types::extraction_job::ExtractionSourceConfig =
                    serde_json::from_value(job.source_config.clone())?;

                if let shared_types::extraction_job::ExtractionSourceConfig::EmailAttachments {
                    email_ids,
                    attachment_types,
                    status_filter: _,
                } = source_config
                {
                    ...
                    if let Some(ids) = email_ids {
                        for email_id in ids {
                            let atts =
                                emails_db::get_email_attachments(db_conn.clone(), email_id).await?;
                    ...
                    extract_from_email_attachments(db_conn.clone(), job_id, attachments, types)
                        .await?;
                }
            }
            _ => {}
    ...
    async fn run_linkedin_extraction_job(db_conn: &AsyncDbConnection, job_id: i64) -> Result<()> {
        let job = jobs_db::get_extraction_job(db_conn.clone(), job_id).await?;

        let source_config: shared_types::extraction_job::ExtractionSourceConfig =
            serde_json::from_value(job.source_config.clone())?;

        if let shared_types::extraction_job::ExtractionSourceConfig::LocalArchive {
            archive_path,
            archive_type,
            files_to_process,
        } = source_config
...
impl AttachmentParserExtractor {
    ...
    fn extract(
        ...
        Vec<shared_types::extraction::ExtractionResult>,
        shared_types::extraction::ExtractionError,
    > {
        ...
        match result.entity {
            ...
            ExtractedEntity::Position(extracted_position) => {
                let company_id = *company_map
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!("Company not found: {}", extracted_position.company_name)
                    })?;
                ...
                let started_date = extracted_position
                    .started_on
                    .as_ref()
                    .and_then(|d| parse_linkedin_date(d));
                let finished_date = extracted_position
                    .finished_on
                    .as_ref()
                    .and_then(|d| parse_linkedin_date(d));
    ...
    for metadata in invitations_metadata {
        let (contact_name, contact_url, invitation_message, invitation_sent_at) =
            if metadata.direction == "OUTGOING" {
                (
                    metadata.to.clone(),
                    metadata.invitee_profile_url.clone(),
                    metadata.message.clone(),
                    Some(metadata.sent_at.clone()),
                )
            } else {
                (
                    metadata.from.clone(),
                    metadata.inviter_profile_url.clone(),
                    metadata.message.clone(),
                    Some(metadata.sent_at.clone()),
                )
            };
