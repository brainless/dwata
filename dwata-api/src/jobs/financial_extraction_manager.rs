use crate::database::{
    emails as emails_db,
    financial_extraction_attempts as attempts_db,
    financial_extraction_sources as sources_db,
    financial_patterns as patterns_db,
    financial_transactions as db,
};
use crate::database::AsyncDbConnection;
use anyhow::Result;
use extractors::FinancialPatternExtractor;
use shared_types::{DataSourceType, Email};
use std::collections::HashMap;

struct AttemptStats {
    total_items_scanned: i64,
    transactions_extracted: i64,
}

struct ProcessResult {
    skipped: bool,
    matches: usize,
    extracted: usize,
}

pub struct FinancialExtractionManager {
    db_conn: AsyncDbConnection,
}

impl FinancialExtractionManager {
    pub fn new(db_conn: AsyncDbConnection) -> Self {
        Self {
            db_conn,
        }
    }

    pub async fn extract_from_emails(
        &self,
        email_ids: Option<Vec<i64>>,
        credential_id: Option<i64>,
    ) -> Result<usize> {
        tracing::info!(
            "Financial extraction start: email_ids={:?}, credential_id={:?}",
            email_ids,
            credential_id
        );

        let source_type = "imap";
        let attempted_at = chrono::Utc::now().timestamp();

        let mut attempt_stats: HashMap<i64, AttemptStats> = HashMap::new();
        if let Some(credential_id) = credential_id {
            attempt_stats.insert(
                credential_id,
                AttemptStats {
                    total_items_scanned: 0,
                    transactions_extracted: 0,
                },
            );
        }

        let db_patterns = patterns_db::list_active_patterns(self.db_conn.clone()).await?;

        tracing::info!(
            "Financial extraction loaded patterns: total_active={}",
            db_patterns.len()
        );

        if db_patterns.is_empty() {
            for (account_id, stats) in attempt_stats {
                let _ = attempts_db::insert_attempt(
                    self.db_conn.clone(),
                    source_type,
                    account_id,
                    attempted_at,
                    stats.total_items_scanned,
                    0,
                    "failed",
                    Some("No active patterns found"),
                )
                .await;
            }

            return Err(anyhow::anyhow!("No active patterns found"));
        }

        let extractor = FinancialPatternExtractor::from_patterns(db_patterns)?;

        let mut total_extracted = 0;
        let mut total_scanned = 0usize;
        let mut total_skipped = 0usize;
        let mut total_matches = 0usize;
        let mut total_loaded = 0usize;

        if let Some(ids) = email_ids {
            let mut emails = Vec::new();
            for id in ids {
                if let Ok(email) = emails_db::get_email(self.db_conn.clone(), id).await {
                    emails.push(email);
                }
            }
            total_loaded = emails.len();
            tracing::info!(
                "Financial extraction loaded emails: total={}, credential_id={:?}",
                total_loaded,
                credential_id
            );
            for email in emails {
                let result = self.process_email(&extractor, &mut attempt_stats, email).await?;
                if result.skipped {
                    total_skipped += 1;
                    continue;
                }
                total_scanned += 1;
                total_matches += result.matches;
                total_extracted += result.extracted;
            }
        } else {
            let batch_size = 1000usize;
            let mut offset = 0usize;
            loop {
                let batch = emails_db::list_emails(
                    self.db_conn.clone(),
                    credential_id,
                    None,
                    batch_size,
                    offset,
                ).await?;

                if batch.is_empty() {
                    break;
                }

                total_loaded += batch.len();
                tracing::info!(
                    "Financial extraction loaded email batch: size={}, offset={}, credential_id={:?}",
                    batch.len(),
                    offset,
                    credential_id
                );

                for email in batch {
                    let result = self.process_email(&extractor, &mut attempt_stats, email).await?;
                    if result.skipped {
                        total_skipped += 1;
                        continue;
                    }
                    total_scanned += 1;
                    total_matches += result.matches;
                    total_extracted += result.extracted;
                }

                offset += batch_size;
            }
        }

        tracing::info!(
            "Financial extraction finished: loaded={}, scanned={}, skipped={}, matched_total={}, extracted_total={}",
            total_loaded,
            total_scanned,
            total_skipped,
            total_matches,
            total_extracted
        );

        for (account_id, stats) in attempt_stats {
            let _ = attempts_db::insert_attempt(
                self.db_conn.clone(),
                source_type,
                account_id,
                attempted_at,
                stats.total_items_scanned,
                stats.transactions_extracted,
                "completed",
                None,
            )
            .await;
        }

        Ok(total_extracted)
    }

    async fn process_email(
        &self,
        extractor: &FinancialPatternExtractor,
        attempt_stats: &mut HashMap<i64, AttemptStats>,
        email: Email,
    ) -> Result<ProcessResult> {
        let stats = attempt_stats.entry(email.credential_id).or_insert(
            AttemptStats {
                total_items_scanned: 0,
                transactions_extracted: 0,
            },
        );
        stats.total_items_scanned += 1;

        let data_source_type = "email";
        let data_source_id = email.id.to_string();

        let is_processed = sources_db::is_source_processed(
            self.db_conn.clone(),
            data_source_type,
            &data_source_id,
        ).await.unwrap_or(false);

        if is_processed {
            tracing::debug!("Skipping already processed email: {}", email.id);
            return Ok(ProcessResult {
                skipped: true,
                matches: 0,
                extracted: 0,
            });
        }

        let text = format!("{}\n\n{}",
            email.subject.unwrap_or_default(),
            email.body_text.unwrap_or_default()
        );

        let results = extractor.extract_from_text(&text);
        let transaction_count = results.len();

        if let Some(stats) = attempt_stats.get_mut(&email.credential_id) {
            stats.transactions_extracted += transaction_count as i64;
        }

        let mut extracted = 0usize;
        for (mut transaction, pattern_id) in results {
            transaction.data_source_type = DataSourceType::Email;
            transaction.data_source_id = data_source_id.clone();

            db::insert_financial_transaction(
                self.db_conn.clone(),
                &transaction,
                None,
            ).await?;

            patterns_db::increment_match_count(
                self.db_conn.clone(),
                pattern_id,
            ).await?;

            patterns_db::update_last_matched(
                self.db_conn.clone(),
                pattern_id,
                chrono::Utc::now().timestamp(),
            ).await?;

            extracted += 1;
        }

        if transaction_count > 0 {
            sources_db::mark_source_processed(
                self.db_conn.clone(),
                data_source_type,
                &data_source_id,
                None,
                transaction_count as i32,
            ).await?;
        }

        Ok(ProcessResult {
            skipped: false,
            matches: transaction_count,
            extracted,
        })
    }
}
