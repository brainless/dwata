...
export default function FinancialHealth() {
  ...
  const fetchFinancialData = async () => {
    ...
    try {
      ...
      const [summaryResponse, transactionsResponse] = await Promise.all([
        fetch(getApiUrl(`/api/financial/summary?start_date=${dates.start}&end_date=${dates.end}`)),
  ...
  const categoryBreakdown = (): CategoryBreakdown[] => {
    const all = transactions();
    const totals = new Map<TransactionCategory, { amount: number; count: number }>();
    ...
    <div class="p-4 md:p-8">
      ...
      <div class="stats stats-vertical lg:stats-horizontal shadow mb-8 w-full">
        ...
        <div class="stat">
          ...
          <div class="stat-desc">
            {summary()!.overdue_payments} overdue, {summary()!.pending_bills} pending
          </div>
        </div>
      </div>
          ...
          <div class="card bg-base-100 shadow-sm border border-base-300">
            <div class="card-body">
               ...
               >
                 <div class="space-y-3">
                  ...
                  <div class="flex items-start gap-3 p-3 rounded-lg bg-base-200 hover:bg-base-300 transition-colors">
                    <div
                      ...
                      classList={{
                        "bg-error/20 text-error": bill.status === "overdue",
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="text-xs text-base-content/60 flex items-center gap-1 mt-1">
                        ...
                        Due {new Date(bill.transaction_date).toLocaleDateString()}
                      </div>
                      {bill.notes && (
                        <div class="text-xs text-error mt-1">{bill.notes}</div>
                      )}
                    </div>
                 ...
                 <button class="btn btn-sm btn-block mt-4">View All Bills</button>
               </Show>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col items-center justify-center py-16 px-4">
          <div class="text-center max-w-md">
            ...
            <p class="text-base-content/60 mb-6">
              Upload your invoices, bills, bank statements, and receipts to get
              started tracking your financial health.
            </p>
            <button class="btn btn-primary gap-2">
              <HiOutlineArrowUpTray class="w-5 h-5" />
