    ...
    let mut stmt = match conn.prepare(
        ...
        Err(_) => {
            let mut legacy_stmt = conn.prepare(
                "SELECT id, name, regex_pattern, description, document_type, status,\n                    amount_group, vendor_group, date_group, is_default, is_active,\n                    match_count, last_matched_at, created_at, updated_at\n             FROM financial_patterns\n             WHERE is_active = true",
            )?;
            ...
            let patterns = legacy_stmt
                .query_map([], |row| {
                    Ok(FinancialPattern {
                        ...
                        status: row.get(5)?,
                        amount_group: row.get::<_, i64>(6)? as usize,
                        vendor_group: row.get::<_, Option<i64>>(7)?.map(|v| v as usize),
